...
test('/proactive formats scheduler jobs and quiet state', () => {
  ...
  try {
    ...
    const text = formatProactiveText(
      [
        { module: 'modulus-google-calendar', name: 'event-reminders', cron: '*/5 * * * *' },
        { module: 'followups', name: 'sweep', cron: '* * * * *' },
      ],
      prefs,
      42,
...
test('/nudges filters recent nudge history by current chat id', () => {
  ...
  try {
    db.prepare(
      `INSERT INTO nudge_log (chat_id, module, job, key, reason, sent_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
...
test('/why shows most recent nudge explanation for current chat id only', () => {
  ...
  try {
    ...
    assert.match(text, /module: followups/);
